...
// Test/verification hook (works in packaged builds too): when TMAX_FORCE_UPDATE=1
// the macOS updater treats the latest GitHub release as newer than the running
// version, so testers can trigger the full download+swap flow without waiting
// for an actual version bump. Never set in normal use.
const forceUpdate = process.env.TMAX_FORCE_UPDATE === '1';
...
export class VersionChecker {
  ...
  start(): void {
    ...
    if (forceUpdate) {
      console.log('[update] TMAX_FORCE_UPDATE=1: latest release will be treated as newer');
    }
    if (process.platform === 'win32' && this.supportsAutoUpdate) {
      // Windows keeps the native Squirrel auto-updater.
      this.setupAutoUpdater();
    } else if (process.platform === 'darwin' && app.isPackaged) {
      // macOS uses a DMG-swap updater: Squirrel.Mac can't update an ad-hoc-signed
      // app, so we download the .dmg and swap the bundle ourselves.
  ...
  restartAndUpdate(): void {
    if (process.platform === 'win32' && this.supportsAutoUpdate && this.updateInfo.status === 'downloaded') {
  ...
  private setupAutoUpdater(): void {
    ...
    autoUpdater.on('error', (err: Error) => {
      ...
      this.broadcastUpdate();
      this.closeFeedServer();
      // Windows-only path now; fall back to a manual GitHub download link.
      this.checkGitHub();
    });

    // setupAutoUpdater is Windows-only (macOS uses the DMG-swap updater).
  ...
  private async checkMacUpdate(): Promise<void> {
    try {
      ...
      // forceUpdate (TMAX_FORCE_UPDATE=1) skips the "is it actually newer?" gate
      // so testers can trigger the whole flow against the current latest release.
      if (!forceUpdate && this.compareVersions(tagName, currentVersion) <= 0) return;
  ...
  private applyMacUpdate(): void {
    ...
    // app quitting. It logs to the log file itself, so stdio is ignored.
    const child = spawn('/bin/bash', [scriptPath, appBundle, dmg, mountPoint, logPath], {
      detached: true,
      stdio: 'ignore',
    });
    child.unref();
